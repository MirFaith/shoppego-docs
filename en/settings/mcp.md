# MCP

Shoppego now allows you to connect Claude or ChatGPT to your store, enabling it to create and manage your pages, forms, themes and products.

The MCP connector URL for your store is as follows: https://\[namasubdomain].myshoppegram.com/mcp

## On this page:

* [Settings on Shoppego](mcp.md#settings-on-shoppego)
* [Connecting ChatGPT](mcp.md#connecting-chatgpt)
* [How to use it on ChatGPT](mcp.md#how-to-use-it-on-chatgpt)
* [Connecting Claude](mcp.md#connecting-claude)
* [Additional information](mcp.md#additional-information)

## Settings on Shoppego

1. Log in to the Shoppego dashboard

<figure><img src="../../.gitbook/assets/lCN7aw3FznaFFoOAyya2.jpg" alt=""><figcaption></figcaption></figure>

2. Click on **Settings**

<figure><img src="../../.gitbook/assets/VsK9zs6aZtUfKK1hcCiM.jpg" alt=""><figcaption></figcaption></figure>

3. Click on **General**

<figure><img src="../../.gitbook/assets/anealodqyXwHTuzr4AHU.jpg" alt=""><figcaption></figcaption></figure>

4. Change the MCP setting to "**On**" and **Save** the setting

<figure><img src="../../.gitbook/assets/dfRI2hqzuLf6EyxwnvAV.jpg" alt=""><figcaption></figcaption></figure>

## Connecting ChatGPT

1. Log in to your ChatGPT account.

<figure><img src="../../.gitbook/assets/NH2OL0cNlf1BRWmnrYsw.jpg" alt=""><figcaption></figcaption></figure>

2. Click on your "**Profile Menu**" and select "**Settings**"

<figure><img src="../../.gitbook/assets/Ghp1I4yXF4m4jJ4u6f7D.jpg" alt=""><figcaption></figcaption></figure>

3. Click on the "**Security and login**" menu

<figure><img src="../../.gitbook/assets/GLFEe7xUuHal0oCXVglT.jpg" alt=""><figcaption></figcaption></figure>

4. Activate the "**Developer mode**" setting toggle

<figure><img src="../../.gitbook/assets/mKZdi29Dd3A2SHKeVqCG.jpg" alt=""><figcaption></figcaption></figure>

5. Then, you can refresh your ChatGPT dashboard page and click the "**Plugins**" menu.

<figure><img src="../../.gitbook/assets/dTgDSxFIIsZLwlQhj3Bn.jpg" alt=""><figcaption></figcaption></figure>

6. On the "**Plugins**" page, you can click the **+ icon** button to add your own plugins.

<figure><img src="../../.gitbook/assets/qQrpuXY7wcGM8Nrk2yY1.jpg" alt=""><figcaption></figcaption></figure>

7. You can enter all your MCP information. For "**Authentication**," you can leave it set to "**OAuth**." Once finished, click **Create**.

<figure><img src="../../.gitbook/assets/uwYPe7RzuT1pBZ3kqqWt.jpg" alt=""><figcaption></figcaption></figure>

8. Later you will be taken to the Shoppego page for you to log in, select the store and approve the setting.

<figure><img src="../../.gitbook/assets/BivCk8r0wTjbWkboa4Tw.jpg" alt=""><figcaption></figcaption></figure>

## How to Use It on ChatGPT

1. Log in to your ChatGPT account.

<figure><img src="../../.gitbook/assets/NH2OL0cNlf1BRWmnrYsw.jpg" alt=""><figcaption></figcaption></figure>

2. Click on "**New chat**"

<figure><img src="../../.gitbook/assets/qslrVnyVZYSEOOiBdRft.jpg" alt=""><figcaption></figcaption></figure>

3. Then, you can click on "**Plugin**" and select the Shoppego plugin you just added.

<figure><img src="../../.gitbook/assets/RK1t0T5bDGzxXNUW3r5b.jpg" alt=""><figcaption></figcaption></figure>

5. Once done, you can write your own prompt to have it make changes or manage your forms or pages.

#### Example Prompt:

<figure><img src="../../.gitbook/assets/06k152mkQyCk8j8p907W.jpg" alt=""><figcaption></figcaption></figure>

#### Prompt Result:

<figure><img src="../../.gitbook/assets/Kbmk9icSMTnMioP4jygA.png" alt=""><figcaption></figcaption></figure>

## Connecting Claude

1. Log in to your Claude account and click on "**Settings**" > "**Connectors**".[https://claude.ai/customize/connectors](https://claude.ai/customize/connectors)

<figure><img src="../../.gitbook/assets/YtcVWQlUQqE3ja5URQyX.jpg" alt=""><figcaption></figcaption></figure>

2. Click the "**Add**" button and select "**Add custom connector**".

<figure><img src="../../.gitbook/assets/J23ZuYqlrjhKcnB5T5Cb.jpg" alt=""><figcaption></figcaption></figure>

3. Once done, you can **enter your MCP information**. For the "**OAuth**" settings, you can **leave both fields blank**. After that, click **Add**.

<figure><img src="../../.gitbook/assets/kSbXGILi3tGa0H0efpvO.jpg" alt=""><figcaption></figcaption></figure>

## Additional Information

For any pages or forms created using this MCP, you cannot make any custom changes to the page content via the builder editor or similar tools.

You can obtain your own MCP server by going to:\
**Shoppego Dashboard > Settings > Custom domain > Copy your subdomain URL**. Then, **add "/mcp" to the end of that subdomain URL**.

**Your MCP server URL should look like this: https://\[subdomainname].myshoppegram.com/mcp**
